...
namespace ABMC_Clientes.GUI
{
    public partial class frmEstadisticaCantidadFacturadaMes : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            rpvCantidadMes.LocalReport.DataSources.Add(new ReportDataSource("dstEstadistica", odato.ConsultarTabla("DATENAME(MONTH, DATEADD(MONTH, Month(f.fecha)-1, '1900-01-01')) as  mes, YEAR(f.fecha) as año, SUM(d.precio) as cobrado", "Facturas f JOIN FacturasDetalle d on (d.id_factura = f.id_factura)", "f.borrado = 0 and d.borrado = 0 Group by MONTH(f.fecha), YEAR(f.fecha)")));
        ...
        private void btnImprimir_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                rpvCantidadMes.LocalReport.DataSources.Add(new ReportDataSource("dstEstadistica", oDat.ConsultarTabla("DATENAME(MONTH, DATEADD(MONTH, Month(f.fecha)-1, '1900-01-01')) as  mes, YEAR(f.fecha) as año, SUM(d.precio) as cobrado", "Facturas f JOIN FacturasDetalle d on (d.id_factura = f.id_factura)", "f.borrado = 0 and d.borrado = 0 AND f.fecha BETWEEN '"+dtpFechaDesde.Value.ToString("yyyy-MM-dd hh:mm:ss") +"' AND '"+dtpFechaHasta.Value.ToString("yyyy-MM-dd hh:mm:ss") +"' Group by MONTH(f.fecha), YEAR(f.fecha)")));
                rpvCantidadMes.RefreshReport();
