    ...
    public partial class frmReporteFacturas : Form
    {
        ...
        private void btnImprimir_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                facturasBindingSource.DataSource = oDat.ConsultarTabla("f.id_factura, f.numero_factura, c.razon_social as cliente, f.fecha, u.usuario as usuario_creador, (SELECT SUM(precio) from FacturasDetalle fd where f.id_factura = fd.id_factura) as Total",
                                                                       "from Facturas f join Usuarios u on(f.id_usuario_creador = u.id_usuario) join clientes c on(f.id_cliente = c.id_cliente)",
                                                                       "WHERE borrado = 0 AND c.fecha_alta BETWEEN '" + dtpFechaDesde.Value.ToString("yyyy-MM-dd hh:mm:ss") + "' AND '" + dtpFechaHasta.Value.ToString("yyyy-MM-dd hh:mm:ss") + "'");
                this.reportViewer1.RefreshReport();
            }
        }
    }
